    ...
    for rep in range(1, int(config["replicates"]) + 1):
        for stim in stimuli_for_split(config["stimulus_split"]):
            for rel_id in relationship_ids():
                ...
                system, user = render_generation_prompts(stim, rel_id)
                rows.append({
...
def generate(client: Any, config: dict[str, Any], row: dict[str, Any], schema: dict[str, Any]) -> dict[str, Any]:
    ...
    system, user = render_generation_prompts(stim, row["rel_id"])
    ...
    response = client.responses.create(
        ...
        store=False,
        text={"format": {"type": "json_schema", "name": "aprl_pf_exp_0006_experience", "strict": True, "schema": schema}},
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run PF-EXP-0006 main generation.")
