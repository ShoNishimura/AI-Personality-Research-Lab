...
def stimuli_for_split(split: str) -> list[dict[str, Any]]:
    return [r for r in design()["stimuli"] if r["split"] == split]


def relationship_by_id(rel_id: str) -> dict[str, Any]:
    entries = design()["relationships"]
    if rel_id not in entries:
        raise ValueError(f"unknown Relationship condition: {rel_id}")
    return {"id": rel_id, **entries[rel_id]}


def relationship_ids() -> tuple[str, str]:
    return ("REL-T", "REL-D")


def values_beliefs_control() -> str:
    return str(design()["values_beliefs_control"])


def render_generation_prompts(stimulus: dict[str, Any], rel_id: str) -> tuple[str, str]:
    ...
    user = (ROOT / "prompts/task.md").read_text(encoding="utf-8").format(
        ...
        values_beliefs=values_beliefs_control(),
...
def render_evaluator_prompts(row: dict[str, Any]) -> tuple[str, str]:
    ...
    user = (ROOT / "prompts/evaluator-task.md").read_text(encoding="utf-8").format(
        situation=row["situation"], perception=row["perception"], experience=row["experience"]
    )
    return system, user
