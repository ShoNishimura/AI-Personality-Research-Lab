...
def evaluate_stimulus(client: Any, config: dict[str, Any], row: dict[str, Any], schema: dict[str, Any]) -> dict[str, Any]:
    ...
    response = client.responses.create(
        ...
        text={
            "format": {
                "type": "json_schema",
                "name": "aprl_pf_exp_0002_stimulus_pretest",
                "strict": True,
                "schema": schema,
            }
        },
...
def print_summary(manifest: list[dict[str, Any]], results_path: Path) -> int:
...
def run(config_path: Path, dry_run: bool) -> int:
        ...
        for attempt in range(int(config["max_retries"]) + 1):
            try:
                ...
                if attempt >= int(config["max_retries"]):
                    print(
                        f"[{index}/{len(manifest)}] failed {row['pretest_id']}: {error_type}",
                        file=sys.stderr,
                    )
    ...
    missing = print_summary(manifest, results_path)
    return 0 if missing == 0 else 1


def main() -> int:
    parser = argparse.ArgumentParser(description="Blind pretest PF-EXP-0002 stimuli.")
