...
def render_evaluator_prompts(interpretation: str) -> tuple[str, str]:
    ...
    task = (ROOT / "prompts/evaluator-task.md").read_text(encoding="utf-8").format(interpretation=interpretation)
    return system, task


def render_pretest_prompts(stimulus: str) -> tuple[str, str]:
    system = (ROOT / "prompts/pretest-system.md").read_text(encoding="utf-8")
    task = (ROOT / "prompts/pretest-task.md").read_text(encoding="utf-8").format(stimulus=stimulus)
    return system, task
