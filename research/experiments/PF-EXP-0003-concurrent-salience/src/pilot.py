...
def execute_run(client: Any, config: dict[str, Any], row: dict[str, Any], schema: dict[str, Any]) -> dict[str, Any]:
    ...
    response = client.responses.create(
        ...
        text={
            "format": {
                "type": "json_schema",
                "name": "aprl_pf_exp_0003_interpretation",
                "strict": True,
                "schema": schema,
            }
        },
        ...
        for attempt in range(int(config["max_retries"]) + 1):
            try:
                ...
                if attempt >= int(config["max_retries"]):
                    print(f"[{index}/{len(manifest)}] failed {row['run_id']}: {error_type}", file=sys.stderr)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run PF-EXP-0003 pilot generation.")
